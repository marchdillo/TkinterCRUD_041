...
class NilaiApp:
    def __init__(self, root):
        ...
        frm_left = ttk.LabelFrame(root, text='Input Data', padding=(8, 8))
        ...
        # Label status untuk menampilkan mode (tambah/edit)
        self.lbl_status = ttk.Label(frm_left, text='Mode: Tambah Data Baru', foreground='blue')
        self.lbl_status.grid(row=4, column=0, pady=8, sticky='w')
        ...
        self.btn_update = ttk.Button(btn_frame, text='Update', command=self.on_update, state='disabled')
        self.btn_update.grid(row=0, column=1, padx=(0, 8))

        self.btn_delete = ttk.Button(btn_frame, text='Delete', command=self.on_delete, state='disabled')
    ...
    def on_update(self):
        if self.selected_id is None:
            messagebox.showwarning('Update', 'Pilih data dari tabel terlebih dahulu.')
            return
        ...
        messagebox.showinfo('Sukses', f'Data berhasil diupdate. Prediksi: {prediksi}')
    ...
    def on_delete(self):
        if self.selected_id is None:
            messagebox.showwarning('Delete', 'Pilih data dari tabel terlebih dahulu.')
            return
    ...
    def on_tree_select(self, event):
        ...
        if selection:
            ...
            self.lbl_status.config(text=f'Mode: Edit Data (ID: {self.selected_id})', foreground='green')
    ...
    def clear_form(self):
        ...
        self.lbl_status.config(text='Mode: Tambah Data Baru', foreground='blue')
